...
AUTO_SITES_HELPER="${SCRIPT_DIR}/modules/lib/monitor_auto_sites.py"
...
monitor_auto_sites() {
    ...
    if [[ ! -x "$AUTO_SITES_HELPER" ]]; then
        log_error "缺少自动生成脚本: $AUTO_SITES_HELPER"
        return 1
    fi

    local monitor_file="${SALTGOAT_MONITOR_FILE:-${SCRIPT_DIR}/salt/pillar/monitoring.sls}"
    local auto_args=(
        --site-root "${SALTGOAT_SITE_ROOT:-/var/www}"
        --nginx-dir "${SALTGOAT_NGINX_DIR:-/etc/nginx/sites-enabled}"
        --monitor-file "$monitor_file"
        --script-dir "$SCRIPT_DIR"
    )
    if [[ "${SALTGOAT_SKIP_SALT_CALL:-0}" =~ ^(1|true|TRUE)$ ]]; then
        auto_args+=(--skip-salt-call)
    fi
    if [[ "${SALTGOAT_SKIP_SYSTEMCTL:-0}" =~ ^(1|true|TRUE)$ ]]; then
        auto_args+=(--skip-systemctl)
    fi

    local monitor_output
    if ! monitor_output=$(python3 "$AUTO_SITES_HELPER" "${auto_args[@]}"); then
    ...
    local added_sites removed_sites updated_items changed=0
    added_sites="$(grep -E '^ADDED_SITES ' <<<"$monitor_output" | cut -d' ' -f2- | tr -d '\r')"
    removed_sites="$(grep -E '^REMOVED_SITES ' <<<"$monitor_output" | cut -d' ' -f2- | tr -d '\r')"
    updated_items="$(grep -E '^UPDATED ' <<<"$monitor_output" | cut -d' ' -f2- | tr -d '\r')"
    if [[ -n "$added_sites" && "$added_sites" != "NONE" ]]; then
        changed=1
    fi
    if [[ -n "$removed_sites" && "$removed_sites" != "NONE" ]]; then
        changed=1
    fi
    if [[ -n "$updated_items" && "$updated_items" != "NONE" ]]; then
        changed=1
    fi

    if (( changed == 0 )); then
        log_info "站点配置未发生变化，跳过 Telegram 话题与 Pillar 刷新。"
        return 0
    fi
