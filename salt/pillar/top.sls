...
{%   endif %}
{% endfor %}
{% set extra_secret_files = ['magento_api', 'restic', 'smtp', 'telegram', 'magento-schedule', 'dropbox'] %}
...
{% for secret in secret_includes %}
    - {{ secret }}
{% endfor %}
{% for name in ['auto', 'magento_api', 'restic', 'smtp', 'telegram', 'magento-schedule', 'dropbox'] %}
