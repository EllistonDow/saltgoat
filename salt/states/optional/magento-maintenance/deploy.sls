# Magento 维护：完整部署（基于 monthly 流程，可复用）
...
{% if not site_name %}
magento_maintenance_deploy_missing_site:
  test.fail_without_changes:
    - comment: "pillar['site_name'] 未提供，无法执行部署任务。"
{% elif not site_path %}
magento_maintenance_deploy_missing_path:
  test.fail_without_changes:
    - comment: "pillar['site_path'] 未提供且无法推导站点路径。"
{% elif not site_exists %}
magento_maintenance_deploy_site_absent:
  test.fail_without_changes:
    - comment: "站点目录 {{ site_path }} 不存在。"
{% elif not magento_exists %}
magento_maintenance_deploy_magento_absent:
  test.fail_without_changes:
    - comment: "未找到 {{ site_path }}/bin/magento，确认 Magento 是否已安装。"
{% else %}

magento_maintenance_deploy_enable:
  cmd.run:
    - name: sudo -u {{ magento_user }} {{ php_bin }} bin/magento maintenance:enable
    - cwd: {{ site_path }}

magento_maintenance_deploy_cleanup:
  cmd.run:
    - name: |
