...
{% if not site_name %}
magento_maintenance_disable_missing_site:
  test.fail_without_changes:
    - comment: "pillar['site_name'] 未提供，无法禁用维护模式。"
{% elif not site_path %}
magento_maintenance_disable_missing_path:
  test.fail_without_changes:
    - comment: "pillar['site_path'] 未提供且无法推导站点路径。"
{% elif not site_exists %}
magento_maintenance_disable_site_absent:
  test.fail_without_changes:
    - comment: "站点目录 {{ site_path }} 不存在。"
{% elif not magento_exists %}
magento_maintenance_disable_magento_absent:
  test.fail_without_changes:
...
magento_maintenance_disable:
  cmd.run:
    ...
    - cwd: {{ site_path }}
    - onlyif: sudo -u {{ magento_user }} {{ php_bin }} bin/magento maintenance:status | grep -q "Maintenance mode is enabled"
