#!/bin/bash
# Magento 2 Salt 维护脚本
# salt/states/scripts/magento-maintenance-salt.sh
...
health_check() {
    ...
    if sudo -u www-data php bin/magento setup:db:status >/dev/null 2>&1; then
        ...
        if echo "$db_status" | grep -q "Declarative Schema is not up to date"; then
            ...
            if sudo -u www-data php bin/magento setup:upgrade; then
                log_success "[SUCCESS] 数据库升级完成"
                log_info "2.2. 清理升级后的缓存..."
                if sudo -u www-data php bin/magento cache:clean; then
                    log_success "[SUCCESS] 缓存清理完成"
                else
                    log_warning "[WARNING] 缓存清理失败"
                ...
                log_error "[ERROR] 数据库升级失败"
            fi
        else
            log_error "[ERROR] 数据库连接异常: $db_status"
        fi
    fi
