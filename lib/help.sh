...
show_nginx_help() {
    ...
    echo ""
    echo "操作:"
    echo "  create <site> <domains> [path] - 创建新站点"
    ...
    echo "  reload                    - 重新加载配置"
    echo "  test                      - 测试配置"
    echo "  add-ssl <site> <domains> [email] - 添加 SSL 证书（未配置邮箱时会提示输入）"
    ...
    echo "  saltgoat nginx create site1 \"example1.com www.example1.com\""
    echo "  saltgoat nginx add-ssl site1 \"example1.com www.example1.com\""
    echo "  saltgoat nginx add-ssl site1 \"example1.com www.example1.com\" admin@example1.com"
...
show_magetools_help() {
    ...
    echo ""
    echo "🔧 权限管理:"
    echo "  permissions fix [path] - 修复Magento权限"
    echo "  permissions check [path] - 检查权限状态"
    echo "  permissions reset [path] - 重置权限"
    echo ""
    echo "  💡 权限管理最佳实践:"
