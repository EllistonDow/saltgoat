...
state_rollback() {
    ...
    # 查找最新的备份
    local latest_backup
    latest_backup=$(find /tmp -name "saltgoat_backup_${state_name}_*" -type d | sort | tail -1)
...
state_backup() {
    local state_name="$1"
    local backup_dir
    backup_dir="/tmp/saltgoat_backup_${state_name}_$(date +%Y%m%d_%H%M%S)"
...
show_state_status() {
    ...
    echo "备份状态:"
    local backup_count
    backup_count=$(find /tmp -name "saltgoat_backup_*" -type d 2>/dev/null | wc -l)
