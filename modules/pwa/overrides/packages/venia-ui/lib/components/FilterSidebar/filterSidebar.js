...
const FilterSidebar = props => {
        ...
        if (group == 'price') {
            ...
            for (const key of uniqueKeys) {
                if (key == 'price[filter]') {
                    ...
                    filterVar.add(item);
                    filterVar && filterState.set('price', new Set(filterVar));
                }
            }
        }
    });
    ...
    const filtersList = useMemo(() => {
        const entries = Array.from(filterItems);
        const priceIndex = entries.findIndex(([group]) => group === 'price');
        if (priceIndex > -1) {
            const [priceEntry] = entries.splice(priceIndex, 1);
            entries.push(priceEntry);
        }

        return entries.map(([group, items], iteration) => {
